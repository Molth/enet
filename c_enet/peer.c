...
*/
void enet_peer_data(ENetPeer *peer, void *data)
{
  peer->data = data;
}
...
/** Recomputes the peer packet throttle based on the current round trip time.
    ...
    @retval 1 if the throttle was increased
    @retval -1 if the throttle was decreased
    @retval 0 otherwise
*/
...
/** Frees all outgoing commands in the given queue and releases their packets.
    @param peer the peer whose commands are being freed
    @param queue the queue of outgoing commands to clear
*/
static void
...
/** Removes and frees a range of incoming commands from a queue, optionally keeping one command.
    @param peer the peer whose commands are being removed
    @param queue the queue to remove from
    @param startCommand the first command to remove
    @param endCommand the end of the range to remove
    @param excludeCommand a command to keep, or NULL
*/
static void
...
/** Removes and frees all incoming commands in the given queue.
    @param peer the peer whose commands are being freed
    @param queue the queue of incoming commands to clear
*/
static void
...
/** Queues an outgoing command for transmission to a peer.
    ...
    @param offset the byte offset of the current fragment within the packet
    @param length the length in bytes of the current fragment payload
...
/** Dispatches any complete unreliable incoming commands on a channel to the application.
    ...
    @param queuedCommand the incoming command to start dispatching from
*/
...
/** Queues an incoming command for processing or reassembly.
    ...
    @param data the payload data of the command, or NULL if none
