...
/** Compares two ENet addresses for equality.
    @param address1 The first address.
    @param address2 The second address.
    @retval 1 if the addresses are equal
    @retval 0 otherwise
*/
static int
_enet_address_equal(const ENetAddress *address1, const ENetAddress *address2)
{
  return memcmp(&address1->inner, &address2->inner, sizeof(_NativeSocketAddress)) == 0;
}

/** Checks whether an ENet address refers to a broadcast address.
    @param address The address to check.
    @retval 1 if the address is a broadcast address
    @retval 0 otherwise
*/
static int
_enet_address_is_broadcast(const ENetAddress *address)
{
  return address->inner.sin4.sin4_family == _ADDRESS_FAMILY_INTER_NETWORK_V4 &&
         address->inner.sin4.sin4_addr == ENET_HOST_BROADCAST;
}
/** Returns the size in bytes of the payload of the given protocol command.
    @param commandNumber The command number.
    @returns The payload size of the command.
*/
size_t
...
/** Changes the state of a peer.
    @param host The host owning the peer.
    @param peer The peer whose state is changing.
    @param state The new peer state.
*/
static void
...
/** Changes the state of a peer and dispatches the appropriate event to the application.
    @param host The host owning the peer.
    @param peer The peer whose state is changing.
    @param state The new peer state.
*/
...
/** Dispatches any queued incoming commands as events to the application.
    @param host The host whose commands are being dispatched.
...
/** Notifies the application that a peer has connected.
    @param host The host owning the peer.
...
/** Notifies the application that a peer has disconnected.
    @param host The host owning the peer.
...
/** Frees sent unreliable commands from the given queue.
    @param peer The peer whose commands are being freed.
    @param sentUnreliableCommands The queue of sent unreliable commands.
*/
static void
...
/** Finds a sent reliable command matching the given sequence number and channel.
    @param list The list of sent reliable commands.
    @param reliableSequenceNumber The reliable sequence number to match.
    @param channelID The channel to match.
    @returns The matching outgoing command, or NULL if not found.
*/
...
/** Removes a sent reliable command matching the given sequence number and channel.
    @param peer The peer whose command is being removed.
    @param reliableSequenceNumber The reliable sequence number to match.
    @param channelID The channel to match.
    @returns The removed command, or NULL if not found.
*/
...
/** Handles a received CONNECT command, allocating a peer for the connecting host.
    @param host The host that received the command.
    @param header The protocol header.
    @param command The connect command.
    @returns The peer created for the connection, or NULL on failure.
*/
static ENetPeer *
    ...
    if (currentPeer->state == ENET_PEER_STATE_DISCONNECTED)
    {
      if (peer == NULL)
        peer = currentPeer;
             ...
             _enet_address_equal(&currentPeer->address, &host->receivedAddress))
    {
...
/** Handles a received reliable SEND command, queueing the packet data.
    @param host The host that received the command.
    @param peer The peer the command was received from.
    @param command The send command.
    @param currentData Pointer to the remaining payload data.
*/
static int
...
/** Handles a received unsequenced SEND command, queueing the packet data.
    @param host The host that received the command.
    @param peer The peer the command was received from.
    @param command The send command.
    @param currentData Pointer to the remaining payload data.
...
/** Handles a received unreliable SEND command, queueing the packet data.
    @param host The host that received the command.
    @param peer The peer the command was received from.
    @param command The send command.
    @param currentData Pointer to the remaining payload data.
...
/** Handles a received fragment of a reliable SEND command, queueing it for reassembly.
    @param host The host that received the command.
    @param peer The peer the command was received from.
    @param command The send fragment command.
    @param currentData Pointer to the remaining payload data.
*/
static int
...
/** Handles a received fragment of an unreliable SEND command, queueing it for reassembly.
    @param host The host that received the command.
    @param peer The peer the command was received from.
    @param command The send fragment command.
    @param currentData Pointer to the remaining payload data.
...
/** Handles a received PING command.
    @param host The host that received the command.
    @param peer The peer the command was received from.
    @param command The ping command.
*/
static int
...
/** Handles a received BANDWIDTH_LIMIT command.
    @param host The host that received the command.
    @param peer The peer the command was received from.
    @param command The bandwidth limit command.
*/
static int
...
/** Handles a received THROTTLE_CONFIGURE command.
    @param host The host that received the command.
    @param peer The peer the command was received from.
    @param command The throttle configure command.
*/
static int
...
/** Handles a received DISCONNECT command.
    @param host The host that received the command.
    @param peer The peer the command was received from.
...
/** Handles a received ACKNOWLEDGE command, confirming delivery of reliable commands.
    @param host The host that received the command.
    @param event The event to fill.
    @param peer The peer the command was received from.
    @param command The acknowledge command.
*/
static int
...
/** Handles a received VERIFY_CONNECT command, completing the connection handshake.
    @param host The host that received the command.
    @param event The event to fill.
    @param peer The peer the command was received from.
    @param command The verify connect command.
*/
static int
...
/** Processes and dispatches all commands received in a single datagram.
    @param host The host that received the commands.
    @param event The event to fill.
...
enet_protocol_handle_incoming_commands(ENetHost *host, ENetEvent *event)
{
  ...
  else
  {
    ...
    if (peer->state == ENET_PEER_STATE_DISCONNECTED ||
        ...
        (!_enet_address_equal(&host->receivedAddress, &peer->address) &&
         !_enet_address_is_broadcast(&peer->address)) ||
...
/** Receives and processes incoming datagrams from the host socket.
    @param host The host receiving the datagrams.
    @param event The event to fill.
    @returns 0 on success, < 0 on failure
*/
static int
...
/** Sends all queued acknowledgements for a peer.
    @param host The host sending the acknowledgements.
    @param peer The peer whose acknowledgements are being sent.
*/
static void
...
/** Checks the peer for timeouts and disconnects it if it has timed out.
    @param host The host whose peer is being checked.
...
/** Checks the outgoing commands of a peer for those that have timed out or been sent too many times.
    @param host The host owning the peer.
    @param peer The peer whose commands are being checked.
    @param sentUnreliableCommands The queue to which sent unreliable commands are moved.
*/
static int
...
/** Sends all outgoing commands of all connected peers.
    @param host The host whose commands are being sent.
    @param event The event to fill.
    @param checkForTimeouts Whether to check for timeouts during the send.
    @returns 0 on success, < 0 on failure
*/
static int
